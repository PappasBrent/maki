...
void MacroForest::MacroExpands(const clang::Token &MacroNameTok,
        ...
        if (Args != nullptr) {
                ...
                for (unsigned int i = 0; i < Args->getNumMacroArguments();
                     i++) {
                        // Before expanding each argument, we backup the
                        // invocation stack, clear it, and add the current
                        // invocation's parent to it. We do this in order to
                        // maintain our invariant that the invocation stack only
                        // ever contain the parent or prior siblings of the
                        // current invocation. If we did not clear the stack
                        // between arguments, then the stack might contain
                        // invocations nested under a previous sibling argument,
                        // which would violate our invariant.
                        ...
                        if (!ArgTokens.empty()) {
                                ...
                                // Remove the last token since it will always be
                                // the EOF token for this argument
                                Arg.Tokens.pop_back();
                        }
                        ...
                        for (auto Tk : MI->tokens())
                                if (clang::Lexer::getSpelling(Tk, SM, LO) ==
                                    Arg.Name.str())
                                        Arg.NumExpansions++;

                        // Add the argument to the list of arguments for this
                        // expansion
        ...
        if (!MI->tokens_empty()) {
                ...
                for (auto &&Arg : Expansion->Arguments) {
                        if (clang::Lexer::getSpelling(MI->tokens().front(), SM,
                                                      LO) == Arg.Name.str())
                                Expansion->ArgDefBeginsWith = &Arg;
                        if (clang::Lexer::getSpelling(MI->tokens().back(), SM,
                                                      LO) == Arg.Name.str())
