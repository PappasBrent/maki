...
AST_POLYMORPHIC_MATCHER_P2(alignsWithExpansion,
        ...
        if (!Expansion->SpellingRange.fullyContains(NodeExE)) {
                if (debug) {
                        llvm::errs()
                                << "Node mismatch <exp end not in expansion "
                                   "spelling range>\n";
                        if (DSTL.ST)
                                DSTL.ST->dumpColor();
                        else if (DSTL.D)
        ...
        if (!frontAligned) {
                if (debug) {
                        ...
                        llvm::errs() << "NodeSpB: ";
                        NodeSpB.dump(SM);
                        auto L1 = SM.getImmediateMacroCallerLoc(
                                Node.getBeginLoc());
                        auto L2 = SM.getImmediateMacroCallerLoc(L1);
                        auto L3 = SM.getImmediateMacroCallerLoc(L2);
                        auto L4 = SM.getImmediateMacroCallerLoc(L3);
                        llvm::errs() << "Imm macro caller loc x1 (macro id: "
                                     << L1.isMacroID() << "): ";
                        L1.dump(SM);
                        llvm::errs() << "Imm macro caller loc x2 (macro id: "
                                     << L2.isMacroID() << "): ";
                        L2.dump(SM);
                        llvm::errs() << "Imm macro caller loc x3 (macro id: "
                                     << L3.isMacroID() << "): ";
                        L3.dump(SM);
                        llvm::errs() << "Imm macro caller loc x4 (macro id: "
                                     << L4.isMacroID() << "): ";
                        L4.dump(SM);
                        ...
                        if (Expansion->ArgDefBeginsWith &&
                            !(Expansion->ArgDefBeginsWith->Tokens.empty())) {
                                llvm::errs()
                                        << "ArgDefBeginsWith front token loc: ";
...
AST_POLYMORPHIC_MATCHER_P2(isSpelledFromTokens,
                           AST_POLYMORPHIC_SUPPORTED_TYPES(clang::Decl,
                                                           clang::Stmt,
                                                           clang::TypeLoc),
                           clang::ASTContext *, Ctx, std::vector<clang::Token>,
                           Tokens) {
        ...
        clang::SourceRange ExpImmMacroCallerRange(
                SM.getExpansionLoc(NodeImmMCB), SM.getExpansionLoc(NodeImmMCE));
        ...
        for (auto Tok : Tokens)
                if (!(SpellingRange.fullyContains(
                      ...
                      ExpImmMacroCallerRange.fullyContains(
                        ...
                        if (DSTL.ST && debug) {
                                llvm::errs()
                                        << "Node mismatch <token not in range>\n";
                                DSTL.ST->dumpColor();
        ...
        if (DSTL.ST) {
                ...
                while (!descendants.empty()) {
                        ...
                        // TODO: Might want to return false instead of
                        // continuing here
                        ...
                        clang::SourceRange ExpImmMCRange(CurImmMCExB,
                                                         CurImmMCExE);
                        ...
                        if (!(TokFileRange.fullyContains(FileRange) ||
                              TokExpRange == ExpImmMCRange)) {
                                if (debug) {
                                        llvm::errs()
                                                << "Node mismatch <descendant>:\n";
                                        DSTL.ST->dumpColor();
                                        llvm::errs()
                                                << "Descendant did not match:\n";
                                        cur->dumpColor();
                                        ...
                                        SM.getSpellingLoc(cur->getBeginLoc())
                                                .dump(SM);
                                        llvm::errs()
                                                << "Desc spelling end loc: ";
                                        SM.getSpellingLoc(cur->getEndLoc())
                                                .dump(SM);
                                        llvm::errs()
                                                << "Desc imm macro caller loc: ";
                                        SM.getImmediateMacroCallerLoc(
                                                  cur->getBeginLoc())
                                                .dump(SM);
                                        llvm::errs()
                                                << "Desc imm macro caller end loc: ";
                                        SM.getImmediateMacroCallerLoc(
                                                  cur->getEndLoc())
                                                .dump(SM);
                                        llvm::errs() << "Token loc: ";
